...
class TextCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.fc(x)
        return x
def load_model(model_path, vocab_size = 10000, embed_dim = 200, numfilters = 100 ,numclasses = 2, kernel_size = [2, 3, 4]):
    model = TextCNN(
...
def predict_sentiment(text, model, vocab, preprocessor = None, max_length=10):
    ...
    encoded_text = preprocessor.encode_text(text, vocab=vocab, max_length=max_length) # load preprocessor from src/preprocessing/preprocessing.py
